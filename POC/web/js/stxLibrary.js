...
(function(){
	...
	function _stxLibrary_js(_stxThirdParty,_exports) {
		...
		*/
		 ...
		 * This visualization will only work if the `extended` parameter in the {@link STXChart#layout} object is set to `true`, 
		 ...
		 * 
		 * Visual Reference:<br>
		 * ![extendedHours](extendedHours.png "extendedHours")
		 * 
		 * @param {STXChart} stx The chart object
		 ...
		 * // now your chart will be highligted as defined.
		 ...
		 * @example
		 * 	// sample code for turning on and off sessions on the chart. 
			...
			stx.append("initializeDisplay", function(){
				...
				if(styles.session){
					...
					for(var i=0;i<this.chart.dataSegment.length;i++){
						...
						var c=null;
						var s=styles.session[m.getSession(ds.DT)];
					...
					for(var panel in this.panels){
						...
						for(i=0;i<ranges.length;i++){
							...
							if(!noDashes && ranges[i].start>this.chart.left) this.plotLine(ranges[i].start, ranges[i].start, this.panels[panel].bottom, this.panels[panel].top, styles.divider.backgroundColor, "line", this.chart.context, this.panels[panel], {
								pattern: "dashed",
								lineWidth: styles.divider.width
							});
