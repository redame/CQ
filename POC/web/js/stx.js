...
(function(){
	...
	function _stx_js(_exports, _stxThirdParty) {
		...
		*/
		 ...
		 * @return A date that falls some where in a matching time segment. Probably
		 * 1 before close.
		 * @memberOf  STX.Market
		 * @since  05-2016-10
		 * @private
		...
		STX.Market.prototype.disableSession = function(session_name, inverted) {
			...
			if (session_name) {
				...
				for (var i = 0;i < this.extraHours.length; i++) {
		 ...
		 * @param {String} Optional session_name If defined search using intraday rules.
		 * Otherwise return the date time for the first enabled session on that date.
		 * @return {Date} Close date/time for the trading session or null if the market is
		 * closed for the given date.
		 * @memberOf  STX.Market
		 * @since  05-2016-10
		 */
		STX.Market.prototype.getClose = function(date, session_name) {
			var d = new Date(date);
		...
		STX.Market.prototype.getNextOpen = function(date) {
			var d = this._find_next_segment(date);
			if (this.zseg_match.adjacent_parent) {
				return this.getNextOpen(d);
			}
		 ...
		 * enabled.
		 * @param [Date=now] date The date on which to check.
		 * @param {String} Optional session_name If a session name is passed in then 
		 * check for the open date time using intraday rules otherwise return the 
		 * date time of first open session of the date regardless of the date.
		 * @return {Date} A date time for the open of a session or null if the market is
		 * closed for the given date.
		 * @memberOf  STX.Market
		 * @since  05-2016-10
		 */
		STX.Market.prototype.getOpen = function(date, session_name) {
			var d = new Date(date);
			if (typeof session_name !== 'undefined') {
				if (this._wasOpenIntraDay(d)) {
					if (this.zseg_match.name == session_name) {
		...
		STX.Market.prototype.getPreviousClose = function(date) {
			var d = this._find_prev_segment(date, false);
			if (this.zseg_match.adjacent_child) {
		...
		STX.Market.prototype.getPreviousOpen = function(date) {
			var d = this._find_prev_segment(date, true);
			if (this.zseg_match.adjacent_parent) {
			...
			for (var i = 0; i < market.rules.length; i++) {
				...
				try {
					if (typeof rule.dayofweek !== 'undefined') {
						rule.year = "*";
						var rd = _TimeSegmentS._createDayOfWeekSegment(market, rule);
						if (p_rule) {
		...
		STX.Studies.DialogHelper=function(params){
			...
			for(var i in libraryEntry.inputs){
				...
				input.value=(sd && sd.inputs && (sd.inputs[i] || sd.inputs[i]===false))?sd.inputs[i]:libraryEntry.inputs[i];
		...
		*/
		...
		STX.StorageManager.get=function(key, cb){
			if(!STX.localStorage) return null;
                ...
                httpRequest.open('POST', "http://localhost:8080/POC/rest/UserService/load",false);
                httpRequest.send();
                var datum = httpRequest.responseText;
			...
			return datum;}
                ...
                /*STX.StorageManager.get=function(key, cb){
                    //alert("Jai ho");
                    var httpRequest = new XMLHttpRequest();
                    httpRequest.open('POST', "http://localhost:8080/POC/rest/UserService/load");
                    httpRequest.send();
